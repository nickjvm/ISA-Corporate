<?php
	function get_all_newsletters() {
		global $connection;
		$query = "SELECT * FROM stc_news.newsletter ORDER BY id DESC";
	...
	function get_articles_for_newsletter($news_id) {
		global $connection;
		$query = "SELECT * FROM stc_news.articles WHERE newsletter_id = {$news_id} ORDER BY sort_order ASC";
	...
	function get_date_for_newsletter($news_id) {
		global $connection;
		$query = "SELECT date FROM stc_news.newsletter WHERE id = {$news_id}";
	...
	function get_selected_newsletter($news_id) {
		global $connection;
		$query = "SELECT * FROM stc_news.newsletter WHERE id = {$news_id}";
	...
	function get_date($news_id) {
			global $connection;
			$query = "SELECT * FROM stc_news.newsletter ";
	...
	function newsletter_date($news_id) {
		global $connection;
		$query = "SELECT date FROM stc_news.newsletter WHERE id= {$news_id} LIMIT 1";
		$newsletter_date = mysql_query($query, $connection);
		confirm_query($newsletter_date);
	...
	function get_id_by_date($news_date) {
		global $connection;
		$query = "SELECT * FROM stc_news.newsletter WHERE date= '{$news_date}'";
	...
	function get_promar_title_by_id($article_id) {
			global $connection;
			$query = "SELECT * FROM stc_news.articles ";
	...
	function get_newsletter_id($news_id) {
		global $connection;
		$query = "SELECT id";
		$query .= " FROM stc_news.newsletter WHERE id= {$news_id} LIMIT 1";
		$newsletter_date = mysql_query($query, $connection);
		confirm_query($newsletter_date);
	...
	function selected_promar_article($article_id) {
		global $connection;
		$query = "SELECT * FROM stc_news.articles WHERE id={$article_id} LIMIT 1";
