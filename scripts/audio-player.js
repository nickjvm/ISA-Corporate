...
var audioplayer_swfobject=function(){
	var d="undefined",
		...
		N,g,M=true,
		m=function(){
			var AA=typeof J.getElementById!=d&&typeof 
			J.getElementsByTagName!=d&&typeof 
			J.createElement!=d,AH=T.userAgent.toLowerCase(),
			...
			if(typeof T.plugins!=d&&typeof T.plugins[s]==R){
				...
				if(typeof o.ActiveXObject!=d){
					try{
						var AD=new ActiveXObject(w);if(AD){AB=AD.GetVariable("$version");
						if(AB){
			...
			(function(){
				if(j){
					...
					function v(){
						...
						var AA=c(R);AA.setAttribute("type",Q);
						...
						if(z&&typeof z===R){
							for(var AI in z){
								if(typeof AM.flashvars!=d){
									AM.flashvars+="&"+AI+"="+z[AI]}else{AM.flashvars=AI+"="+z[AI]
								}
							}
						}if(f(y)){
							var AN=U(AJ,AM,AH);
...
var AudioPlayer=function(){
	...
	return{
		...
		O.playerID=I;
		audioplayer_swfobject.embedSWF(F,I,N.width.toString(),"24",G,false,O,J,M);H.push(I)
		...
		if(J==undefined){
			J=1}B(I).open(J==undefined?0:J-1)
